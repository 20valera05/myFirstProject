...
const taskContainerTop = document.getElementById("taskContainerTop");
const taskContainerBottom = document.getElementById("taskContainerBottom");
...
function saveDivToLocalStorage(divId) {
    const divContent1 = document.getElementById(divId).innerHTML;
    localStorage.setItem(divId, divContent1);
}

function loadDivFromLocalStorage(divId) {
    const divContent2 = localStorage.getItem(divId);
    if (divContent2) {
        document.getElementById(divId).innerHTML = divContent2;
    }
}

window.onload = function () {
    taskInput.focus();
    loadDivFromLocalStorage("taskContainerTop");
    loadDivFromLocalStorage("taskContainerBottom");
    const quantityOfTaskTopFromStorage = localStorage.getItem('quantityOfTaskTop');
    const quantityOfTaskBottomFromStorage = localStorage.getItem('quantityOfTaskBottom');
    if (quantityOfTaskTopFromStorage !== null && quantityOfTaskBottomFromStorage !== null) {
        quantityOfTaskTop = parseInt(quantityOfTaskTopFromStorage);
        quantityOfTaskBottom = parseInt(quantityOfTaskBottomFromStorage);
        allEmptyDiv();
        updateDivAll();
    }
};
...
function updateDivAll() {
    quantityOfTaskTop = taskContainerTop.querySelectorAll("div");
    quantityOfTaskBottom = taskContainerBottom.querySelectorAll("div");
    allDiv.innerHTML = quantityOfTaskTop.length + quantityOfTaskBottom.length;
    bottomDiv.innerHTML = quantityOfTaskBottom.length;
    const countOfTask = parseFloat(quantityOfTaskBottom.length / (quantityOfTaskTop.length + quantityOfTaskBottom.length)) * 100;
    const intCountOfTask = parseInt(countOfTask);
...
function allEmptyDiv() {
    ...
    if (quantityOfTaskTop.length === 0 && quantityOfTaskBottom.length === 0) {
        ...
        const noTaskContainer = document.getElementById("noTaskContainer");
        if (noTaskContainer) {
    ...
    localStorage.setItem('quantityOfTaskTop', quantityOfTaskTop.length);
    localStorage.setItem('quantityOfTaskBottom', quantityOfTaskBottom.length);
}
...
function createTask(event) {
    ...
    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        let topContainerHTML = taskContainerTop.innerHTML;
        localStorage.setItem('taskContainerTop', topContainerHTML);
    }
}

taskInput.addEventListener("keydown", createTask);
...
function openModalWindow(removeParentDiv) {
    ...
    const spanElemText = removeParentDiv.querySelector("span");
    ...
    confirmButton.addEventListener("click", function () {
        if (removeParentDiv) {
            ...
            const topContainerHTML = taskContainerTop.innerHTML;
            localStorage.setItem('taskContainerTop', topContainerHTML);

            const bottomContainerHTML = taskContainerBottom.innerHTML;
            localStorage.setItem('taskContainerBottom', bottomContainerHTML);
        }
        modalWindow.classList.add("hidden");
...
document.addEventListener("click", function (event) {
    if (event.target && event.target.tagName === "A") {
        ...
        const spanElem = parentDiv.querySelector("span");
        if (parentContainer === taskContainerTop) {
            if (spanElem) {
                spanElem.classList.add("line-through");
            }
            (event.target).classList.remove("hover:bg-green-200", "border-green-500");
            (event.target).classList.add("bg-green-400", "border-green-600");
            taskContainerBottom.insertAdjacentElement("afterbegin", parentDiv);

            const topContainerForBottom = taskContainerBottom.innerHTML;
            localStorage.setItem('taskContainerBottom', topContainerForBottom);
            const topContainerCheck = taskContainerTop.innerHTML;
            localStorage.setItem('taskContainerTop', topContainerCheck);
        }
        if (parentContainer === taskContainerBottom) {
            if (spanElem) {
                spanElem.classList.remove("line-through");
            }
            (event.target).classList.remove("bg-green-400", "border-green-600");
            (event.target).classList.add("hover:bg-green-200", "border-green-500");
            taskContainerTop.insertAdjacentElement("beforeend", parentDiv);

            const bottomContainerCheck = taskContainerBottom.innerHTML;
            localStorage.setItem('taskContainerBottom', bottomContainerCheck);
            const bottomContainerForTop = taskContainerTop.innerHTML;
            localStorage.setItem("taskContainerTop", bottomContainerForTop);
        }
        allEmptyDiv();
        updateDivAll();
    }
